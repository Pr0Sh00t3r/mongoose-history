...
describe("History Model", function() {
    describe("historyCoolectionName", function() {
        it("should set a collection name", function(done) {
            var collectionName = hm.historyCollectionName(
                "original_collection_name",
                "defined_by_user_history_collection_name"
            );
            collectionName.should.eql(
                "defined_by_user_history_collection_name"
            );
            done();
        });
    ...
    it("could have own indexes", function(done) {
        var HistoryPost = Post.historyModel();
        HistoryPost.collection.indexInformation({full: true}, function(
            err,
            idxInformation
        ) {
    ...
    it("could have another connection", function(done) {
        ...
        post.save(function(err) {
            should.not.exists(err);
            secondConn.db.collection("posts_another_conn_history", function(
                err,
                hposts
            ) {
                should.not.exists(err);
