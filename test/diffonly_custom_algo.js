...
describe("History plugin custom diff algo", function() {
    ...
    function createAndUpdatePostWithHistory(post, newTags, callback) {
        post.save(function(err) {
            ...
            Post.findOne(function(err, post) {
                ...
                post.save(function(err) {
                    if (err) return callback(err);
                    HistoryPost.findOne({"d.title": "Title changed"}, function(
                        err,
                        hpost
                    ) {
                        should.exists(hpost);
    ...
    it("should keep insert in history", function(done) {
        post.save(function(err) {
            should.not.exists(err);
            HistoryPost.findOne({"d.title": "Title test"}, function(
                err,
                hpost
            ) {
                should.not.exists(err);
    ...
    it("should not care about order of tags", function(done) {
        ...
        createAndUpdatePostWithHistory(post, newTags, function(
            err,
            post,
            hpost
        ) {
    ...
    it("should keep just what changed in update", function(done) {
        should.exists(post);
        createAndUpdatePostWithHistory(post, defaultTags, function(
            err,
            post,
            hpost
        ) {
    ...
    it("should keep remove in history", function(done) {
        ...
        createAndUpdatePostWithHistory(post, defaultTags, function(
            ...
            post.remove(function(err) {
                should.not.exists(err);
                HistoryPost.find({o: "r"})
                    .select("d")
                    .exec(function(err, historyWithRemove) {
